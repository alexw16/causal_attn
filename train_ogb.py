...
def main():
    ...
    edge_loss_weights = addl_params['edge_loss_weights']
    ...
    optimizer_file_name = '{}.init.{}heads.{}hd.nl{}.optimizer.pt'.format(args.model_type,args.K,
                                                           args.dim_hidden,args.n_layers)
    if os.path.exists(os.path.join(args.save_dir,model_file_name)) \
        and os.path.exists(os.path.join(args.save_dir,optimizer_file_name)):
        print('Loading initial model + optimizer...')
        model.load_state_dict(torch.load(os.path.join(args.save_dir,model_file_name)))
        optimizer.load_state_dict(torch.load(os.path.join(args.save_dir,optimizer_file_name)))
        ...
        torch.save(model.state_dict(),os.path.join(args.save_dir,model_file_name))
        torch.save(optimizer.state_dict(),os.path.join(args.save_dir,optimizer_file_name))
    ...
    model_file_name = '{}.base.{}heads.{}hd.nl{}.pt'.format(args.model_type,args.K,
                                                       args.dim_hidden,args.n_layers)
    if not os.path.exists(os.path.join(args.save_dir,model_file_name)):
    ...
    intervention_loss = 'interv' in args.model_type
